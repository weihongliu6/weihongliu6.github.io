...
function bookPage(book){
  if(book.status!=='sample') return `<section class="coming-page"><a class="back-link" href="#/">← 返回书架</a><div class="coming-cover book-object">${cover(book,true)}</div><p class="eyebrow">THE COLLECTION</p><h1>${e(book.title)}</h1><p class="coming-label">即将进入数字书房</p><p>本书的数字阅读内容尚未收录。</p>${book.appleBooksTitle?`<p class="sample-note">Apple Books 版题名：《${e(book.appleBooksTitle)}》</p>`:''}${book.appleBooksUrl?`<div>${appleBooksLink(book,'primary-link')}</div>`:''}<a class="primary-link" href="#/">返回数字书房 <span aria-hidden="true">→</span></a></section>`;
